...
//				+ "values(:nomArticle, :descriptions, :dateDebutEncheres, :dateFinEncheres, :miseAPrix, :prixVente, :idUtilisateur, :idCategorie)";
	...
	public List<Article> afficherArticlesFiltres(String filtreNomArticle, int categorieFiltree, String encheresEnCours,
		...
		String sql = "SELECT a.idArticle, a.photoArticle, a.nomArticle, "
				+ "a.descriptions, a.dateDebutEncheres, a.dateFinEncheres, "
				+ "a.miseAPrix, a.prixVente, a.idUtilisateur, a.idCategorie, "
				+ "a.etatVente, r.rue, r.codePostal, r.ville "
				+ "FROM article a LEFT JOIN retrait r ON a.idArticle = r.idArticle "
				+ "LEFT JOIN enchere e ON a.idArticle = e.idArticle WHERE "
				+ "(:filtreNomArticle IS NULL OR a.nomArticle "
				+ "LIKE CONCAT('%', :filtreNomArticle, '%')) "
				...
				+ "AND (:encheresRemportees = 0 OR a.idUtilisateur = :encheresRemportees "
				+ "AND a.etatVente = 'ET') "
				+ "AND (:ventesEnCours = 0 OR a.idUtilisateur = :ventesEnCours "
				+ "AND a.etatVente = 'EC') "
				+ "AND (:ventesEnAttente = 0 OR a.idUtilisateur = :ventesEnAttente "
				+ "AND a.etatVente = 'CR') "
				+ "AND (:ventesTerminees = 0 OR a.idUtilisateur = :ventesTerminees "
				+ "AND a.etatVente = 'ET') "
				+ "GROUP BY a.idArticle, a.photoArticle, a.nomArticle, a.descriptions, "
				+ "a.dateDebutEncheres, a.dateFinEncheres, "
				+ "a.miseAPrix, a.prixVente, a.idUtilisateur, a.idCategorie, a.etatVente, "
				+ "r.rue, r.codePostal, r.ville";
